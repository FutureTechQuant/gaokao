...
def score_budget_text(text: str, url: str) -> int:
    score = 0
    keywords = {
        "单位预算": 8,
        "部门预算": 7,
...
def extract_items(source: dict, html: str) -> list[dict]:
    ...
    for a in soup.select("a[href]"):
        ...
        score = score_budget_text(text, href)
        if source.get("must_include") and not any(k in text for k in source["must_include"]) and score < 5:
            continue
        if score <= 0:
            continue
