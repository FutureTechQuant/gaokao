...
def render_source_summary(lines: list[str], source_summary: dict):
    ...
    if by_type:
        lines.append("- 按信源类型：")
        for name, value in sorted(by_type.items(), key=lambda x: (-x[1], x[0])):
            lines.append(f"  - {name}：{value}")

    by_platform = source_summary.get("by_platform", {})
    if by_platform:
        lines.append("- 按平台：")
        for name, value in sorted(by_platform.items(), key=lambda x: (-x[1], x[0])):
            lines.append(f"  - {name}：{value}")

    by_topic = source_summary.get("by_topic", {})
    if by_topic:
        lines.append("- 按专题：")
        for name, value in sorted(by_topic.items(), key=lambda x: (-x[1], x[0])):
            lines.append(f"  - {name}：{value}")
...
def render_readme(latest_payload: dict) -> None:
    ...
    render_count_block(lines, topic_counts.most_common(12), "- 暂无专题数据")
    ...
    render_count_block(lines, platform_counts.most_common(12), "- 暂无平台数据")
    ...
    render_count_block(lines, source_type_counts.most_common(12), "- 暂无信源类型数据")
    ...
    render_count_block(lines, trust_counts.most_common(12), "- 暂无可信度数据")
    ...
    render_count_block(lines, tag_counts.most_common(20), "- 暂无标签数据")
    ...
    if not top_items:
        ...
        for item in top_items:
            ...
            date = item.get("date", "未知日期") or "未知日期"
            lines.append(
                f"- [{title}]({url}) | {category} | {source} | {platform}/{source_type} | {date} | score={score}"
            )
    ...
    if errors:
        ...
        for err in errors[:20]:
            lines.append(
                f"- {err.get('source', 'unknown')} | {err.get('platform', 'unknown')} | {err.get('url', '')} | {err.get('error', '')}"
            )
